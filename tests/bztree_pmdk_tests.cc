...
struct MultiThreadUpsertTest : public pmwcas::PerformanceTest {
  ...
  MultiThreadUpsertTest(uint32_t item_per_thread, uint32_t thread_count,
                        bztree::BzTree *tree)
      : tree(tree),
        thread_count(thread_count),
        item_per_thread(item_per_thread) {}

  void SanityCheck() {
    for (uint32_t i = 0; i < (thread_count)*item_per_thread; i += 1) {
      auto i_str = std::to_string(i);
      uint64_t payload;
      auto rc = tree->Read(i_str.c_str(), static_cast<uint16_t>(i_str.length()),
                           &payload);
      ASSERT_TRUE(rc.IsOk());
...
GTEST_TEST(BztreePMEMTest, MiltiInsertTest) {
  pmwcas::InitLibrary(pmwcas::PMDKAllocator::Create(
                          TEST_POOL_NAME, TEST_LAYOUT_NAME, 1024 * 1024 * 1024),
  ...
  new (bztree->pmwcas_pool)
      pmwcas::DescriptorPool(pool_size, thread_count, false);
